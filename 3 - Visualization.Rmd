...
viz %>% 
  ...
  theme(axis.ticks.x.bottom = element_blank(),
        axis.text.x.bottom = element_text(angle = 45, vjust = 0.5, hjust = 1, size = 15),
  ...
  annotate(geom = "point", 
           x = as.POSIXct("2022-12-08 06:00"), y = 5.5, color = "grey", size = 3, shape = 16, alpha = 0.8) +
  annotate(
  ...
  annotate(geom = "point", 
           x = as.POSIXct("2022-12-08 06:00"), y = 5.1, color = "darkgreen", size = 3, shape = 16) +
annotate(
  ...
  annotate(geom = "point", 
           x = as.POSIXct("2022-12-08 06:00"), y = 4.8, color = "orange", size = 3, shape = 16) +
annotate(
  ...
  annotate(geom = "point", 
           x = as.POSIXct("2022-12-08 06:00"), y = 4.5, color = "red", size = 3, shape = 16) +
...
dfull2 %>% 
  ...
  annotate(geom = "point", 
           x = as.POSIXct("2022-12-06 22:35"), y = 5.5, color = "grey", size = 3, shape = 16, alpha = 0.8) +
  annotation_custom(rasterGrob(morflag, width = unit(1, "npc"), height = unit(1, "npc")),
...
t.test(s$`1`, s$`0` ,alternative="greater", mu=0, var.equal=FALSE, conf.level = 0.95)
...
t.test(s$`1`, s$`0`, alternative="greater",mu=0,var.equal=TRUE)
...
```{r}
s<-split(viz$Proportion,viz$costarica)
...
```{r}
s<-split(viz$Proportion,viz$japan) 
...
```{r}
s<-split(viz$Proportion,viz$germany)
...
res.aov <- aov(Proportion ~ match_which, data = viz)

summary(res.aov) 
TukeyHSD(res.aov, which = "match_which")
