...
```{r}
varfull <- var(viz$Proportion)

varcheck <- viz %>% 
  filter(created_at < "2022-12-05 00:00:00")

varpre <- var(varcheck$Proportion)

variances <- as.data.frame(table(varfull, varpre)) %>% 
  select(-Freq) %>% 
  rename("Variance for full period" = "varfull",
         "Variance for first 3 games" = "varpre") %>% 
  mutate_all(~ as.numeric(as.character(.))) %>%
  mutate(across(everything(), ~ round(., 2)))

kableExtra::kable(variances, format = "pipe", align = "c")
```
...
ggsave("general.jpeg", width = 7.5, height = 4.5, dpi = 300)
...
dfull2 %>% 
  ...
  annotate(geom = "segment", x = as.POSIXct("2022-12-06 22:35"), xend = as.POSIXct("2022-12-06 22:45"), y = 5.2, yend = 5.2, linetype = "dashed", size = 0.2, alpha = 0.3) +
  annotate(
    "text", x = as.POSIXct("2022-12-06 23:05"), y = 5.2,
    label = "Overtime",
    color = "black",
    size = 7
  ) + 
  annotate(geom = "segment", x = as.POSIXct("2022-12-06 22:35"), xend = as.POSIXct("2022-12-06 23:20"), color = "black", alpha = 0.5, size = 0.35, y = 5.33, yend = 5.33) +
  annotate(geom = "point", 
