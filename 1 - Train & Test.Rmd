...
corpus <- corpus(df$text, docvars = data.frame(hate = df$hate, hateterm = df$hateterm))
summary(corpus)
```
...
dfmat_train <- corpus_subset(corpus, id_numeric %in% id_train) %>% tokens() %>% dfm() 
dfmat_test <- corpus_subset(corpus, !(id_numeric %in% id_train)) %>% tokens %>% dfm() 
```
