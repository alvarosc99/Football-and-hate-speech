...
```{r}
library(randomForest)
RF_model = randomForest(docvars(dfmat_train)$hate ~ ., data=dfmat_train)
...
```{r}
rfprobability = predict(rfFit, testing, type="prob")

rfprediction <- as.factor(ifelse(rfprobability[,2] > 0.2, "Yes", "No"))

confusionMatrix(rfprediction, testing$Attrition, positive = "Yes")
```
